###############################################################
#  Kitti to PascalVOC converting script
# 
###############################################################
...
class kitti2voc:
    ...
    def convert_process(self):
        ...
        for filename in os.listdir(self.kitti_anat_path):
            ...
            PascalVocWriter_ins = PascalVocWriter(self.image_path_dst,image_name,image_shape)
            count = 0 
...
if __name__ =='__main__':
    ...
    output_default_path = sys.path[0]
    #Getting user inputs 
    ap=argparse.ArgumentParser()
    ap.add_argument('-ipi','--image_path', required=True, type=str ,help='path to kitti images')
    ap.add_argument('-ipl','--label_path', required=True, type=str ,help='path to kitti labels')
    ap.add_argument('-op', '--ouput_path', default=output_default_path , type=str ,help='path to store output')

    args =vars(ap.parse_args())

    image_path_src = args['image_path']
    kitti_anat_path = args['label_path']
    output_path = args['ouput_path']

    pascal_output_dir = output_path + '/VOCdevkit' + '/VOC2012'
    pathlib.Path(pascal_output_dir).mkdir(parents=True, exist_ok=True) 
    pathlib.Path(pascal_output_dir+'/Annotations').mkdir(parents=True, exist_ok=True) 
    pathlib.Path(pascal_output_dir+'/ImageSets' + '/Main').mkdir(parents=True, exist_ok=True) 
    pathlib.Path(pascal_output_dir+ '/JPEGImages').mkdir(parents=True, exist_ok=True) 

    image_path_dst = pascal_output_dir + '/JPEGImages' 
    voc_anat_path= pascal_output_dir + '/Annotations'

    kitti2voc_ins = kitti2voc(image_path_src,image_path_dst,kitti_anat_path,voc_anat_path)
    kitti2voc_ins.convert_process()
    print ("Kitti to PascalVOC data convertion completed...")
